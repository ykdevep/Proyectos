...
import datetime
import os
import time as t #dddddddddddddddd
...
def db_to_gpx(db,dest):
	...
	with con:
		...
		cur = con.cursor()    
		cur.execute('SELECT track_id, start_time, type from sport_summary where type=5 or type=2 or type=1')
		...
		for running_session in running_sessions:
			...
			cur.execute('SELECT location_data.latitude, location_data.longitude, location_data.altitude, location_data.timestamp from location_data, sport_summary where location_data.track_id=' + str(track_id) + ' and type="' + str(running_session[2]) +'"')
			datos = cur.fetchall()
			...
			# Write Header
				print('File '+year+'-'+month+'-'+day+'_'+hour+'-'+minute+'-'+second+'.gpx   syncing...')
