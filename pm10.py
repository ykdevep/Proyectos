...
##                                                                                                                  ##
##                   Se leen los datos pertenecientes a los valores de contaminación de PM10 (PM10)                 ##
...
nombreBDPm10 = 'bancosexternos'  ## Configuración para la base de dato de contaminantes NO...
config = {
  'user': 'kike',
  'password': 'kike123',
  'host': '127.0.0.1',
...
tablaPm10 = {} ## Definición de la tabla de contaminantes NO...
tablaPm10[nombreBDPm10] = ( 
    "CREATE TABLE `pm10` ("
...
def create_database(cursor):  ## Función para la base de d...
    try:
        cursor.execute(
            "CREATE DATABASE {} DEFAULT CHARACTER SET 'utf8'".format(nombreBDPm10))
    except mysql.connector.Error as err:
        print ("   ")
...
for name, ddl in tablaPm10.items():
    try:
        print ("   ")
...
addPm10 = ("INSERT INTO pm10"
                "(FECHA, HORA, ACO, AJM, ATI, BJU, CAM, CHO, CUA, CUT, FAC, HGM, INN, IZT, MER, MGH, MPA, PED, SAG, SFE, TAH, TLA, TLI, UIZ, VIF, XAL)"
                "VALUES (%(datoFecha)s, %(datoHora)s, %(datoACO)s, %(datoAJM)s, %(datoATI)s, %(datoBJU)s, %(datoCAM)s, %(datoCHO)s, %(datoCUA)s, %(datoCUT)s, %(datoFAC)s, %(datoHGM)s, %(datoINN)s, %(datoIZT)s, %(datoMER)s, %(datoMGH)s, %(datoMPA)s, %(datoPED)s, %(datoSAG)s, %(datoSFE)s, %(datoTAH)s, %(datoTLA)s, %(datoTLI)s, %(datoUIZ)s, %(datoVIF)s, %(datoXAL)s)"
            )
...
for i in range(0, len(xls)):
    ...
    cursor.execute(addPm10, datosPm10)
    cnx.commit()
