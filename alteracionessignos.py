...
tablaVitales = {} ## Definición de la tabla de alteraciones...
...
datosAlter = {
    'datoClasificacion' : clasificacion,
    'datoNombre' : nombre,
    'datoDescripcion' : descripcion,
}

addAlter = ("INSERT INTO alteraciones"
                "(CLASIFICACION, NOMBRE, DESCRIPCION)"
                "VALUES (%(datoClasificacion)s, %(datoNombre)s, %(datoDescripcion)s)"
            )
...
for i in range(0, len(xls)):
    clasificacion = xls.iloc[i,0]
    nombre = xls.iloc[i,1]
    descripcion = xls.iloc[i,2]
    ...
    cursor.execute(addAlter, datosAlter)
    cnx.commit()
