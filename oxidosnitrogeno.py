...
##                                                                                                                  ##
##            Se leen los datos pertenecientes a los valores de contaminación de Óxidos de Nitrógeno (NOX)          ##
...
tablaOxNitr = {} ## Definición de la tabla de contaminantes NO...
tablaOxNitr[nombreBDOxNitr] = ( 
...
def create_database(cursor):  ## Función para la base de d...
    try:
        cursor.execute(
            "CREATE DATABASE {} DEFAULT CHARACTER SET 'utf8'".format(nombreBDOxNitr))
    except mysql.connector.Error as err:
        print ("   ")
...
for name, ddl in tablaOxNitr.items():
    try:
        print ("   ")
...
addNOX = ("INSERT INTO oxnitrogenos"
...
for i in range(0, len(xls)):
    ...
    cursor.execute(addNOX, datosNOX)
    cnx.commit()
